...
- **Manual only.** `translation_audit` jobs are built by `buildTranslationAuditJobs(extensionIds, locales)` and enqueued by `triggerTranslationAudit` on a `TRIGGER_TRANSLATION_AUDIT` message - never by the daily cycle. They are *appended* (no clearing of pending jobs) and never claim a scan slot (`scanCycleSlotKey` stays null). They are invisible to the cycle machinery: the daily "previous cycle still running" guard and the manual-refresh queue clearing both skip `translation_audit` jobs, so a multi-hour audit neither costs the day its scan nor gets silently cancelled by "Refresh Now".
- A locale the extension does not ship (not in the listing's `availableLocales`) is served the default listing. Its snapshot is stored with `isLocalized: false`, excluded from the analysis, and shown as "default listing" - never flag it as untranslated English.
...
- The name check flags **brand loss**, never wording. Which title words are the brand is *inferred* by `identifyBrandTokens`: coined-looking words (digit / inner capital) plus words most of the other audited locales kept verbatim (or that sit inside a non-Latin-script title); untranslated copies of the English title are not evidence. This is why `detectDifferentName` receives the other locales' titles and why flags must be recomputed across all locales when one lands. Don't fall back to "every non-stop-word is the brand" - that flagged honest translations of descriptive titles ("Pinterest Pin Stats - Sort Pins").
- Every flag should carry a human-readable `details` (why it fired) and, for text-block detectors, a **verbatim** `excerpt` - the comparison table highlights the excerpt inside the full description by exact substring match, so don't normalize or reflow the excerpt text.
...
**Service Worker:**
- No `setTimeout`/`setInterval` - use `chrome.alarms` (survives SW termination). Chrome's floor is **`delayInMinutes: 0.5`** (30s) - below that it refuses and warns; it may also fire later than asked, so treat the value as a floor, not a promise. `MIN_ALARM_DELAY_MINUTES` was `1`, twice the real limit, which silently overrode `queueDelayMs`: the Settings slider offers 30s but every request went out 61s apart. A floor stricter than Chrome's own is a bug, not caution. The one exception is `paginationDelay`, a `setTimeout` inside a single job execution, and the `AbortController` timer behind `FETCH_TIMEOUT_MS`. `paginationDelay` waits the *configured* `queueDelayMs` ± `queueJitterMs` — pages 2 and 3 of a search are CWS requests like any other, and a hardcoded 2s gap ignored the pace the user set — so it can no longer lean on "the fetch keeps the worker alive": it sleeps in sub-30s chunks with a `chrome.*` call between them, the only thing that resets Chrome's idle timer. It is also capped (`MAX_PAGINATION_DELAY_MS`), because that sleep happens *inside* a job that stays `running`. Each keep-alive tick of that sleep also refreshes the job's `heartbeatAt` — see the abandoned-job rule below.
- **Abandoned jobs are detected by silence, not age.** A job carries `heartbeatAt` (unindexed; stamped by `dequeueNext`, refreshed on every keep-alive tick of a pagination wait); `requeueStaleRunningJobs` measures from it, falling back to `startedAt` for pre-heartbeat jobs. This is what lets `STALE_RUNNING_JOB_MS` be **3 minutes** instead of 15: the window no longer has to exceed the longest legitimate *job* (a three-page keyword scan pacing itself), only the longest legitimate *silence* (one fetch, bounded by `FETCH_TIMEOUT_MS`). Keep it that way — the window is pure loss on every worker death (the job idles through it, then restarts from page 1), and MV3 kills workers freely. If you add a job type that issues several CWS requests without a paced wait between them, make it beat too.
- **A late-firing `dailyScan` alarm must not run its slot.** `chrome.alarms` delivers an overdue alarm on wake, so a 04:00 slot's alarm can arrive at 09:41. `handleDailyScanAlarm` applies `CATCH_UP_MIN_LEAD_MS` (`lateSlotRefusal`) and declines when the next slot is under 30 minutes out: the late cycle would still be draining when the real slot fires, the in-flight guard would eat that slot, and its snapshots carry the *previous* slot-day's `cycleDate` — a full day of scanning filed under yesterday with today's column empty. Reuse only the lead-time rule, never `isDailyScanDue` wholesale: its "occurred today" check would permanently drop any slot whose jitter carries its alarm past midnight. The refusal falls through to the `finally` that re-arms, or one late delivery ends the alarm chain.
...
- **Every scheduling mechanism here is a self-re-arming chain, and MV3 breaks chains.** `processQueue` is a one-shot alarm re-armed by the handler that consumes it, so a worker terminated mid-fetch stalls the queue for good. The periodic `queueWatchdog` alarm (5 min) is the only thing that reconnects it: it re-queues jobs abandoned in `running`, re-arms `processQueue` when work is pending and nothing is in flight, runs a missed slot via `isDailyScanDue`, and re-arms a missing `dailyScan` alarm. Anything new that depends on an alarm chain must be recoverable from it.
- **An empty `dequeueNext()` is not an empty queue.** It returns only jobs whose `scheduledAt` has arrived, so every remaining job may just be in retry backoff. Reporting that as "cycle finished" stamps the slot, stops the alarm chain and abandons those jobs — whose leftovers then trip the in-flight guard on every later slot. `processNextJob` must keep `hasMore` true and ask to be woken when the earliest backoff expires.
- **A queued job is only valid for its own slot.** Jobs carry `cycleDate`/`slot` so a cycle crossing midnight reports as one day, which also means a stranded job drained days late writes today's measurements into a past date (and leaves today empty). `purgeExpiredCycleJobs` discards pending/running cycle jobs older than one slot interval + 1h grace, on startup and every watchdog tick; `translation_audit` is exempt.
- The "previous cycle still running" guard needs an escape hatch: `isQueueMakingProgress` decides whether the queue is actually moving (recent cycle start / recent completion / a job scheduled recently or in the future). Without it one dead cycle blocks every future slot forever.
- Catch-up (`isDailyScanDue`) gates on the slot's **own occurrence** (`currentSlotOccurrence`), not its slot-day: with `dailyScanTime: 10:00` and `scansPerDay: 4` the 04:00 slot fires today but belongs to yesterday's slot-day, and gating on the slot-day made it permanently uncatchable.
- Scheduled `review_scan` jobs run only on slot 0. Later slots get new reviews via the listing scan's free review count: when it rises, `queueReviewFollowUp` queues one review scan for that extension (deduplicated against a pending/running one).
