...
### Fixed
- **The scan queue could stall permanently, and every later slot was then silently skipped.** Three separate faults, each of which stopped scanning on its own; together they explain missing user/review columns on later days and a "Scanning…" state that never ends.
  - **Jobs waiting out a retry backoff were mistaken for an empty queue.** `dequeueNext` only returns jobs whose `scheduledAt` has arrived, so once every remaining job was in backoff (a CWS 429 is enough — and more frequent at `scansPerDay: 4`) the processor reported "no more work". The scheduler then stamped the slot as complete, broadcast SCAN_COMPLETE and stopped re-arming — abandoning those jobs as `pending` forever. Those orphans then tripped the "previous cycle still running" guard on **every** subsequent slot, every subsequent day. `processNextJob` now distinguishes "queue empty" from "nothing due yet" and asks to be woken when the earliest backoff expires.
  - **Nothing recovered a broken processing chain.** `processQueue` is a one-shot alarm re-armed by the handler that consumes it, so an MV3 worker termination mid-fetch broke the chain for good: the job stayed `running`, no alarm was armed, and only a browser restart ever noticed. A new periodic **watchdog alarm** (`queueWatchdog`, every 5 minutes) re-queues jobs abandoned by a terminated worker, re-arms the processing alarm when work is pending and nothing is in flight, and re-arms a missing `dailyScan` alarm.
  - **A stalled cycle blocked all later slots indefinitely.** The in-flight guard had no upper bound, so leftovers from a dead cycle deferred the current slot, and the next, forever. It now checks whether the queue is actually moving (a recent cycle start, a recently completed job, or a job scheduled recently/in the future) and, when it is not, discards the dead cycle's jobs and takes the slot. A slow-but-healthy cycle still defers as before.
- **A missed scan slot was only caught up on browser startup.** The catch-up ran from `chrome.runtime.onStartup` / extension update alone, so on a machine that stays awake a slot whose alarm never arrived (worker killed between fires, machine asleep past the fire time, alarm lost on a reload) was simply never made up. The watchdog now runs the same catch-up check every 5 minutes: a 10:00 slot missed and noticed at 12:00 runs at 12:00, and the next slot still fires on schedule.
- **After-midnight slots could never be caught up.** `isDailyScanDue` gated on the slot's *slot-day*, but with `dailyScanTime: 10:00` and `scansPerDay: 4` the 04:00 slot fires today while belonging to yesterday's slot-day — so it was rejected as "yesterday's" and skipped every single day. The check now uses the slot's own occurrence (`currentSlotOccurrence`). At `scansPerDay: 1` behaviour is unchanged: a slot missed before midnight still stays missed.
- **A stranded cycle drained days later and wrote today's data into a past date.** Every job carries the `cycleDate` and `slot` its snapshots are recorded under, so a cycle crossing midnight still reports as one day — but with no expiry, a cycle stranded by a broken alarm chain ran whenever the queue was next kicked and recorded *today's* measurements against its original date. What you saw was a rank-change event stamped "Sep 6, 1:12 PM (within ~2.6d)", the Sep 4 column growing a second intraday sample (`#5-6`), and nothing at all in the Sep 5 / Sep 6 columns. Queued cycle jobs are now discarded once a later slot has superseded them (one slot interval plus an hour's grace), on startup and on every watchdog tick: their slot is already being re-scanned at the right time, so the only thing running them could still do is misattribute data. A cycle that is merely crossing midnight, or working through a retry backoff, is untouched; `translation_audit` jobs are exempt entirely.
- **A queued translation audit suppressed the startup catch-up.** `handleBrowserStartup` treated any pending job as an unfinished cycle, and an audit of 150 locale pages sits in the queue for hours — so for as long as it ran, every missed slot was skipped. Audit jobs are excluded from that check (the daily cycle already ignored them); the audit still resumes.

### Notes
- **CWS/proxy requests now time out after 90 seconds.** `fetchPage` was a bare `fetch` with no ceiling, so a stalled proxy or a dropped connection could hang for minutes — and an in-flight fetch keeps the MV3 worker alive, leaving the job `running` with nobody to end it. That breaks the assumption the new recovery rests on (a job `running` past 15 minutes is one a dead worker abandoned): against a *live* slow fetch, re-queuing it means the same job executing twice at once. A timeout makes "running for a quarter of an hour" mean what the scheduler assumes it means. Timeouts are retriable, like any other transport error.
- **The stale-cycle window is derived from the configured pace**, not fixed at 30 minutes: `max(30 min, 3 x (queueDelayMs + queueJitterMs))`. Those settings have a validated minimum but no maximum — only the Settings UI caps them, and a restored backup does not go through the UI — so a fixed window would have made every cycle at a slower configured pace look dead and discard its jobs.
- The watchdog never attempts catch-up while a cycle is draining. `lastScanSlotKey` is stamped only when a cycle *finishes*, so the slot running right now reads as "not run" for its whole duration — without that guard every watchdog tick of a healthy 45-minute scan would have logged "Scan slot … skipped … Lower scansPerDay or queueDelayMs", nine false alarms per scan telling you to undo a setting that works.

### Added
- **Later slots now pick up new reviews the same day.** Scheduled review scans still run only on the day's first slot — refetching the whole review list four times a day multiplies request volume for nothing — but the listing scan that runs on *every* slot already reports the review count for free. When a later slot sees that count rise, a `review_scan` is queued for just that extension, so a review left at 11:00 is captured within the hour instead of waiting for the next day's first slot. No extra request when the number has not moved, and never more than one follow-up queued per extension at a time.
...
### Fixed
- Audit cache key now distinguishes the primary keyword, is collision-proof for keywords with separators, and busts on review-content or custom-prompt changes; review fingerprint keys on review content rather than timestamps; future-dated reviews no longer skew the recent-rating trend.
